...
const TasksSkeleton = React.memo(() => (
  <div className="px-2 xs:px-3 sm:px-4 md:max-w-4xl lg:max-w-5xl md:mx-auto pb-8">
    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-2 sm:gap-3 md:gap-4">
      {Array.from({ length: 6 }).map((_, i) => (
        ...
        >
          <div className="px-2.5 py-2.5 sm:p-4 h-full flex flex-col">
            ...
            <div className="flex items-center justify-between pt-1.5 sm:pt-2 mt-auto">
              {/* Due date skeleton */}
...
const CalendarSkeleton = React.memo(() => (
  <div className="max-w-full md:max-w-5xl mx-auto px-2 md:px-6 mb-3 sm:mb-4">
    ...
    <div className="pb-2 sm:pb-3">
      <div className="grid grid-cols-7 gap-1.5 sm:gap-2 md:gap-3 px-2 md:px-4">
        {Array.from({ length: 7 }).map((_, i) => (
          <div
            key={i}
            className={`flex flex-col items-center justify-center h-14 sm:h-16 md:h-20 lg:h-24 w-full p-1.5 sm:p-2 md:p-3 rounded-lg animate-pulse ${i === 3 ? 'bg-blue-500/80' : 'bg-gray-200 dark:bg-gray-700'
              }`}
            style={{ animationDelay: `${i * 50}ms` }}
          >
            {/* Weekday abbreviation (WED, THU, etc.) */}
            <div className={`w-6 h-2.5 rounded mb-0.5 sm:mb-1 ${i === 3 ? 'bg-blue-200' : 'bg-gray-300 dark:bg-gray-600'
              }`}></div>
            {/* Day number (31, 01, etc.) */}
            <div className={`w-6 h-6 sm:w-7 sm:h-7 rounded ${i === 3 ? 'bg-blue-200' : 'bg-gray-300 dark:bg-gray-600'
              }`}></div>
          </div>
        ))}
      </div>
...
const TaskCard = memo(({
  ...
  return (
    <div
      ...
      `}
    >
      <div className="px-2.5 py-2.5 sm:p-4 flex-grow flex flex-col">
        {/* Header Section */}
        <div className="flex items-start mb-1.5 sm:mb-2">
          <div className="flex items-start gap-1.5 min-w-0 flex-1">
            <h3 className={`
              text-xs xs:text-sm sm:text-base font-semibold leading-tight flex-1 line-clamp-2
            ...
            {hasAttachments && (
              <div className="flex-shrink-0 mt-0.5">
                <Paperclip className="w-2.5 h-2.5 xs:w-3 xs:h-3 sm:w-3.5 sm:h-3.5 text-gray-500 dark:text-gray-400" />
              </div>
            )}
          </div>
        </div>
        ...
        {/* Description */}
        <p className={`
          text-[10px] xs:text-xs sm:text-sm leading-relaxed line-clamp-2 mb-1.5 sm:mb-2 flex-grow
          ${task.status === 'completed'
        ...
        {/* Footer Section */}
        <div className="flex items-center justify-between pt-1.5 sm:pt-2 border-t border-gray-100 dark:border-gray-700/50 gap-1">
          {/* Due Date */}
          <div className="flex items-center gap-0.5 xs:gap-1">
            ...
            />
            <span className={`
              text-[9px] xs:text-[10px] sm:text-xs font-medium truncate
              ${isOverdue && !task.status
                ? 'text-red-500 dark:text-red-400'
                : 'text-gray-500 dark:text-gray-400'
              }`
          ...
          <div className="flex items-center gap-1">
            ...
            {(task.status === 'completed' || isOverdue) && (
              <span className={`
                ...
                text-[8px] xs:text-[9px] sm:text-[10px] font-medium
            ...
            <span className={`
              ...
              text-[8px] xs:text-[9px] sm:text-[10px] font-medium tracking-wide
              rounded-md border flex-shrink-0 min-w-fit
            ...
            `}>
              ...
              <span className="capitalize whitespace-nowrap hidden xs:inline">
...
export function UpcomingPage({ tasks: propTasks, openTaskId, onOpenTaskIdConsumed }: UpcomingPageProps) {
  ...
  return (
    <div className="min-h-screen bg-gray-50 dark:bg-gray-900">
          ...
          <div className="grid grid-cols-7 gap-1.5 sm:gap-2 md:gap-3 px-2 md:px-4">
            {weekDays.map((day) => (
              <button
                ...
                className={`
                flex flex-col items-center justify-center
                h-14 sm:h-16 md:h-20 lg:h-24 w-full
              ...
              `}>
                ...
                {/* Day Number */}
                <span className={`
                text-lg sm:text-xl md:text-2xl lg:text-3xl font-bold mt-0.5 sm:mt-1
                ${day.isSelected
                    ? 'text-white'
                    : day.isToday
                      ? 'text-blue-600 dark:text-blue-400'
      ...
      <div className="px-2 xs:px-3 sm:px-4 md:max-w-4xl lg:max-w-5xl md:mx-auto pb-6 md:pb-8">
