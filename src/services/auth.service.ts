...
export async function loginUser({ email, password }: LoginCredentials): Promise<User> {
  try {
    ...
    if (isDevelopment && testAccounts.includes(email)) {
      ...
      console.log('🔧 [DEV MODE] Created mock user with role:', mockUser.role);
      ...
      localStorage.setItem('nesttask_cached_user', JSON.stringify(mockUser));
...
export async function signupUser({ 
  ...
  try {
    ...
    // Validate department, batch, and section IDs are present
    if (!departmentId || !batchId || !sectionId) {
      console.error('Missing required IDs:', { departmentId, batchId, sectionId });
      throw new Error('Please select your department, batch, and section before signing up');
    }

    // Validate email domain if required for your university
    // Allow test emails and DIU emails
    const isValidEmail = email.endsWith('@diu.edu.bd') || 
                        email.includes('@gmail.com') || 
                        email.includes('@test.com') ||
                        email === 'superadmin@nesttask.com';
    
    if (!isValidEmail) {
      throw new Error('Please use a valid email address (@diu.edu.bd, or test email)');
    }
    ...
    console.log('User created in auth, session:', data.session ? 'Active' : 'Pending confirmation');
...
export async function logoutUser(): Promise<void> {
  try {
    ...
    // Perform Supabase signOut and IndexedDB clear in parallel
    ...
    await Promise.all([signOutPromise, indexedDBPromise]);
    
  } catch (error: any) {
    console.error('Logout error:', error);
    // Don't throw - allow logout to complete even with errors
  }
}
